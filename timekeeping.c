...
struct timespec diff_timespec(struct timespec tm1, struct timespec tm2)
{
 ...
 if ((tm1.tv_sec < tm2.tv_sec) || ((tm1.tv_sec == tm2.tv_sec) && (tm1.tv_nsec <= tm2.tv_nsec))) { /* TIME1 <= TIME2? */
...
void os_tick_update(void)
{
	...
	sub.tv_nsec=0;
		...
		if(node->tmr_c.active==true) {
			...
			diffns = node->tmr_c.ts.tv_nsec - node->tmr_c.rawtime.tv_nsec;
		...
		{
			node->tmr_p.rawtime.tv_nsec += diffts.tv_nsec;

			diffns = node->tmr_p.ts.tv_nsec - node->tmr_p.rawtime.tv_nsec;
			diffns = diffns/1000;
			if (diffns < 0 )  		{
...
{
	long long nsec, sumns;
	struct timespec diffts, ts;
	tmr->active=false;
...
static enum hrtimer_restart hrtimer_thread_callback(struct hrtimer *timer)
{
	...
	for(;;){
                ...
                ovrrun = hrtimer_forward(timer, kt_now, ktime_set(0, SCHED_TIME_NS));
                if(!ovrrun){
                        break;
                }
