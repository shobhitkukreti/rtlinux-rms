/*!
 * \brief 	    Implements SYSCALL Definitons, SYSFS Show and Store functions
...
int rt_flag = 0;
EXPORT_SYMBOL(rt_flag);

struct work_struct sched_hi_work;

/* Work Queue to Schedule our Q Process */
DECLARE_WORK(sched_wq, rtsched_work_handler);
EXPORT_SYMBOL(sched_wq);
...
{
	sched_queue *node;
	node = kzalloc(sizeof(sched_queue), GFP_KERNEL);
	if(node==NULL)
		return NULL;
	node->next=NULL;
	node->cpuq=NULL;
...
static ssize_t 
proc_time_show(struct kobject *kobj, struct attribute *attr, char *buf)
{
	...
	if(node)
		return scnprintf(buf, PAGE_SIZE, "P:%d, RC:%lld.%.9ld RT:%lld.%.9ld, \
		C_ms:%d, T_ms:%d\n", node->pid,(long long)node->rawets.tv_sec,\
		node->rawets.tv_nsec,(long long)node->rawpts.tv_sec,\
		node->rawpts.tv_nsec, node->c_time, node->p_time);
...
/* (BAD IDEA)
 * Setting Reservations from sysfs */
static ssize_t 
proc_time_store(struct kobject *kobj, struct attribute *attr, const char *buf,\
								 size_t len)
{
	int ret, pid, c, t;
	struct proc_attr *sattr;
	...
	ret = sscanf(buf,"%d %d %d", &pid, &c, &t); /* Check for Error */
...
static ssize_t proc_crnt_show(struct kobject *kobj, struct attribute *attr, char *buf)
{

        if(crnttsk)
                return scnprintf(buf, PAGE_SIZE, "P:%d E:%lld.%.9ld,C:\
                 %lld.%.9ld T %lld.%.9ld RC:%lld.%.9ld RT:%lld.%.9ld\n",crnttsk->pid,\
	                (long long)crnttsk->ets.tv_sec, crnttsk->ets.tv_nsec, \
                        (long long)crnttsk->cts.tv_sec, crnttsk->cts.tv_nsec, \
                        (long long)crnttsk->pts.tv_sec, crnttsk->pts.tv_nsec,\
                        (long long)crnttsk->rawets.tv_sec,crnttsk->rawets.tv_nsec,\
                        (long long)crnttsk->rawpts.tv_sec,crnttsk->rawpts.tv_nsec);
...
static struct sysfs_ops crnt_ops = {
	.show = proc_crnt_show,
	.store = NULL,
};
...
static struct proc_attr crnt_info = {
	.attr.name = "current",
	.attr.mode=0644,

};

static struct proc_attr dummy_info = {
	.attr.name = "dummy",
	.attr.mode=0644,
...
static struct attribute *attr_direc[12] = {
	&crnt_info.attr,
	NULL
};
...
void create_pid_sysfs(sched_queue *eq)
{
	...
	pr_err("%s, pid: %d\n", __func__, eq->pid);
	kobject_init(&eq->pid_kobj, &eq->sysfs_file);
...
void force_rt(pid_t next, pid_t prev, struct timespec* exec_value, int val)
{
	...
	if((tmp=check_dup(&runqueue, prev))!=NULL) {
		...
		if(if_tmr_active(&tmp->tmr_c)==true) {
			tmp->e_time = rsv_timer_cancel(&tmp->tmr_c, *exec_value);
		}
		tmp->state=TSK_SUSP;	
...
void inline sys_cleanup(sched_queue *node)
{
...
void rt_exit(pid_t pid)
{
	...
	if((tmp=check_dup(&runqueue, pid))!=NULL) {
		sys_cleanup(tmp);
		tmp->raised_exit=1;
...
/* Protect with Mutex as Queue Structure
 * may change if another syscall adds task
 */
...
asmlinkage long sys_rtexynos(pid_t procid , int comp_time, int period)
{
	...
	if((tmp=check_dup(&runqueue,(pid_t)procid))!=NULL)
        {
              	spin_lock(&(tmp->lock));
	...
	else
	{
		...
		tmp->tmr_c.rawtime.tv_sec=0;
		tmp->tmr_c.rawtime.tv_nsec=0;
		tmp->tmr_c.ts.tv_sec=0;
		tmp->tmr_c.ts.tv_nsec = MS_TO_NS(comp_time);
		...
		tsk->static_prio, tsk->normal_prio, tsk->rt_priority);
		...
		cpu0=NULL;cpu1=NULL;cpu2=NULL;cpu3=NULL;	
		protect =0;
		os_core_timer_init();
	}	
